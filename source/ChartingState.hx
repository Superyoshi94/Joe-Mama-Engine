...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		bpmTxt = new FlxText(1000, 50, 0, "BPM", 16);
	...
	override function update(elapsed:Float)
	{
			...
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps))
		{
			...
			{
				dummyArrow.y = Math.floor(FlxG.mouse.y / 20) * 20;
			}
			if (FlxG.keys.pressed.N)
			{
				dummyArrow.y = Math.floor(FlxG.mouse.y / 13.33333333) * 13.33333333;
			}
			if (FlxG.keys.pressed.M)
			{
				dummyArrow.y = Math.floor(FlxG.mouse.y / 10) * 10;
			}
		}
